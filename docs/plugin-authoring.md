...
## Host-supplied context checkpoint

Real examples may use the small runtime-neutral `context.macro` task facade from `@macro/plugin`. The host owns project identity, cancellation, transport, and authorization. Server reactions also receive a structured logger. Plugin code must not construct an SDK client, read tokens or environment configuration, or implement its own transport. Capability names are descriptive metadata in this POC; runtime enforcement, installation binding, and delegated grants remain host work. See `examples/plugins/task-inbox`.
