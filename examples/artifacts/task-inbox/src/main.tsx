...
function App() {
  ...
  const upsert = (task: TaskRow) => setTasks((current) => {
  ...
  void tokenPromise.then(async (token) => {
    const hosts = __MACRO_STORAGE_HOST__ ? { storage: __MACRO_STORAGE_HOST__ } : undefined;
    macro = new Macro({ token, env: __MACRO_ENV__, hosts, webAppUrl: __MACRO_WEB_APP_URL__ });
    ...
    for await (const event of macro.tasks.subscribe({ projectId: __MACRO_PROJECT_ID__ })) {
  ...
  const createTask = async (event: SubmitEvent) => {
    ...
    if (!macro || !taskName) return;
    await createPrivateTask(macro, __MACRO_PROJECT_ID__, taskName);
    setName('');
  };

  return <main><h1>Task inbox</h1><form onSubmit={createTask}><input aria-label="Task name" value={name()} onInput={(e) => setName(e.currentTarget.value)} /><button type="submit">Create</button></form><Show when={error()}>{(message) => <p class="error">{message()}</p>}</Show><ul><For each={tasks()}>{(task) => <li>{task.name}</li>}</For></ul></main>;
}
