...
export function mountDocumentHealth(
	...
	const connect = async () => {
		if (stopListening || connecting || disposed) return;
		connecting = true;
		...
		try {
			...
			setStatus("Live updates unavailable. Use Refresh to retry.", true);
		} finally {
			connecting = false;
		}
	};
	...
	required(root, "#refresh").addEventListener("click", () => {
		void connect();
		void refresh();
	});
	void connect();
