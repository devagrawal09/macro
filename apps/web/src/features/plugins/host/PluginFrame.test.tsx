...
async function mountFrame(overrides?: {
	...
	return {
		...
		initNonces: () =>
			posted
				.ofType(PLUGIN_INIT_TYPE)
				.map((message) => message.nonce as string),
	};
}
...
describe('PluginFrame', () => {
	it('completes the happy handshake and delivers context plus grant over the port', async () => {
		...
		const connectCall = frame.posted.calls.find(
			(call) =>
				(call[0] as { type?: string }).type === PLUGIN_CONNECT_TYPE,
		);
		...
		expect(
			frame.view.container.textContent,
		).not.toContain('Loading plugin');
		frame.view.unmount();
	});
	...
	it('tears down the old frame and remounts when the mount context changes', async () => {
		const [context, setContext] =
			createSignal<PluginClientContext>(CONTEXT);
		...
		readyFrom(firstIframe, firstPost.ofType(PLUGIN_INIT_TYPE)[0].nonce as string);
		await vi.waitFor(() => expect(channels).toHaveLength(1));
