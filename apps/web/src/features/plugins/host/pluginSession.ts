...
export interface PluginSessionOptions {
   ...
   * Unknown or malformed port messages are logged and ignored; the session
   * stays alive.
   */
...
export function createPluginSession(options: PluginSessionOptions): {
  ...
  const onMessage = async (event: MessageEvent) => {
    ...
    channel.port1.onmessage = (portEvent) => {
      ...
      if (isPluginFatalMessage(portEvent.data)) {
        fail(`plugin-fatal:${portEvent.data.message}`);
        return;
      }
      if (isPluginIntentMessage(portEvent.data)) {
        options.onIntent?.(portEvent.data.intent);
        return;
      }
      console.warn('[plugin-session] ignoring unknown plugin port message');
    };
    try {
