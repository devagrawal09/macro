...
import type { MacroClientCore } from './utils/client-core';

/** Shared facade implementation for the server and browser entrypoints. */
export class MacroBase<C extends MacroClientCore, M> {
  ...
  protected constructor(
    client: C,
    private readonly recreate: (requestedAs: string) => M,
  ) {
    this._client = client;
  ...
  requestedAs(user: User): M {
    return this.recreate(user.id);
  }
}
